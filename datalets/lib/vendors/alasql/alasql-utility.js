...
export const alasql_QUERY = function(data, fields, filters, aggregators, orders)
{
...
export const alasql_transformData = function(data, fields, round)
{
...
let alasql_QUERY_string = function(fields, filters, aggregators, orders)
{
    if (fields) {
...
let _alasql_SELECT = function(fields)
{
    let select = "SELECT ";
...
let _alasql_WHERE = function(filters)
{
    /* OLD VERSION */
    if(!filters[0].field) {
        if (filters.length == 1)
...
let _alasql_GROUPBY = function(aggregators)
{
...
let _alasql_ORDERBY = function(orders)
{
...
let _addParenthesis = function(fields)
{
    var result = [];
...
let _copy = function(o)
{
    ...
    for (key in o) {
        v = o[key];
        out[key] = (typeof v === "object") ? _copy(v) : v;
    }
    return out;
};
